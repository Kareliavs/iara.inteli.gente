...
def calcular(
    ...
    if ind < 0:
        raise ValueError("Indicador 6048 invalido (< 0)")
...
def _calcular_nivel_contexto(
    ...
    for indicador_referencia, indicador_valor, indicador_nivel in rows:
        indicador_ref_int = int(indicador_referencia)
        ...
        nivel_calc = calcular_nivel_maturidade(str(indicador_referencia), float(indicador_valor))
        if nivel_calc is not None:
            niveis_por_indicador[indicador_ref_int] = int(nivel_calc)
