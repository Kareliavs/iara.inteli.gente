...
def calcular(
    ...
    if ind < 0:
        raise ValueError("Indicador 3069 invalido (< 0)")

    if ind == 0:
        return 1

    if ind == 1:
        nivel_3043 = _buscar_nivel_3043(conn, municipio_cod_ibge, ano)
        if nivel_3043 is None:
            return 2
        if nivel_3043 < 2:
            return 2
        if nivel_3043 > 4:
...
def _buscar_nivel_3043(
    ...
    nivel_calculado = calcular_nivel_maturidade("3043", float(indicador_valor))
