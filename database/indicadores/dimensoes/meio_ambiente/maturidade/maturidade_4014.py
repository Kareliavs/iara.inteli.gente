...
def calcular(
    ...
    if ind < 0:
        raise ValueError("Indicador 4014 invalido (< 0)")

    if ind == 0:
        nivel_4007 = _buscar_nivel_4007(conn, municipio_cod_ibge, ano)
        if nivel_4007 is None:
            return 1
        if nivel_4007 < 1:
            return 1
        if nivel_4007 > 2:
...
def _buscar_nivel_4007(
    ...
    nivel_calculado = calcular_nivel_maturidade("4007", float(indicador_valor))
